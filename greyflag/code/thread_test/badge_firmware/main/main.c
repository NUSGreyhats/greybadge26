...
static void joiner_callback(otError error, void *context)
{
    ...
    if (error == OT_ERROR_NONE) {
        printf("[JOINER] join succeeded — attaching to network\n");
        otThreadSetEnabled(inst, true);
    } else {
        printf("[JOINER] join FAILED: %d — retrying in 10s\n", error);
        // retry — joiner will scan again
        otJoinerStart(inst,
                      JOINER_CREDENTIAL,
...
{
    nvs_flash_init();
    // NO esp_netif_init() — disturbs 802.15.4 modem clock refs
