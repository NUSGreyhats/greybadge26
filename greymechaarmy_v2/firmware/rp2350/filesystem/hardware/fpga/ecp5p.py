# circuitpython ESP32S2
# ECP5 JTAG programmer

# AUTHOR=EMARD
# LICENSE=BSD

# prog-only (no flash)
# memory saver that works with ESP32-S2 WROOM

from time import sleep,monotonic_ns
import board, busio, digitalio
from micropython import const
from struct import pack, unpack
from gc import collect

from hardware.fpga import jtag
from hardware.fpga.jtag import *
from hardware.fpga._burst import BurstDriver

# Pin objects used by BurstDriver — must match jtag.gpio_tck/tdi/tdo.
_BURST_PIN_TCK = board.GP19
_BURST_PIN_TDI = board.GP18
_BURST_PIN_TDO = board.GP16
_BURST_FREQ_HZ = 6_000_000

# call this before sending the bitstream
# FPGA will enter programming mode
# after this TAP will be in "shift DR" state and TCK/TDI/TDO have been
# released so the burst driver can claim them for PIO.
def prog_open():
    common_open()
    sir(b"\x46")  # LSC_INIT_ADDRESS
    sdr_idle(b"\x01", 2, 10)
    sir(b"\x7A")  # LSC_BITSTREAM_BURST
    # ---------- bitstream begin -----------
    send_tms(0)   # -> capture DR
    # NOTE: TCK glitch — bitbang_jtag_off releases pins, BurstDriver.open() re-claims them.
    bitbang_jtag_off()

# call this after uploading all of the bitstream blocks,
# this will exit FPGA programming mode and start the bitstream
# returns status True-OK False-Fail
def prog_close():
  bitbang_jtag_on()
  send_tms(1) # -> exit 1 DR
  send_tms(0) # -> pause DR
  send_tms(1) # -> exit 2 DR
  send_tms(1) # -> update DR
  #self.send_tms(0) # -> idle, disabled here as runtest_idle does the same
  runtest_idle(100, 10)
  # ---------- bitstream end -----------
  sir_idle(b"\xC0",2,1) # read usercode
  usercode = bytearray(4)
  sdr_response(usercode)
  check_response(unpack("<I",usercode)[0],expected=0,message="FAIL usercode")
  sir_idle(b"\x26",2,200) # ISC DISABLE
  sir_idle(b"\xFF",2,1) # BYPASS
  sir(b"\x3C") # LSC_READ_STATUS
  status = bytearray(4)
  sdr_response(status)
  status = unpack("<I",status)[0]
  check_response(status,mask=0x2100,expected=0x100,message="FAIL status")
  done = True
  if (status & 0x2100) != 0x100:
    done = False
  reset_tap()
  #self.led.value=0
  bitbang_jtag_input()
  bitbang_jtag_off()
  bitbang_tms_off()
  return done

def prog_stream(filedata, blocksize=4096):
    prog_open()
    bytes_uploaded = 0
    stopwatch_start()
    block = bytearray(blocksize)
    driver = BurstDriver(_BURST_PIN_TCK, _BURST_PIN_TDI, _BURST_PIN_TDO, _BURST_FREQ_HZ)
    driver.open()
    try:
        while True:
            n = filedata.readinto(block)
            if not n:
                break
            if n == blocksize:
                driver.write(block)
            else:
                driver.write(memoryview(block)[:n])
            bytes_uploaded += n
    finally:
        driver.close()
    stopwatch_stop(bytes_uploaded)
    # prog_close() starts with bitbang_jtag_on() which reclaims TCK/TDI/TDO.
    # TMS (GP17) was never released — bitbang_jtag_off() only deints TCK/TDI/TDO.
    # bitbang_jtag_on() now claims TCK at value=False (low), matching the PIO's
    # ending TCK=0 state, so there is no spurious rising edge on handoff.

def prog(filepath, close=True):
  filedata, gz = filedata_gz(filepath)
  if filedata:
    if gz:
      prog_stream(filedata,blocksize=4096)
    else:
      prog_stream(filedata,blocksize=4096)
    # NOTE now the SD card can be released before bitstream starts
    if close:
      return prog_close() # start the bitstream
    return True
  return False
