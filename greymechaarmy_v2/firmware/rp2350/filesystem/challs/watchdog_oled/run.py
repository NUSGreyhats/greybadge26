...
def print_forever(uart):
    print("WDOG_OLED: UART_PRINT_FOREVER")
    while True:
        chunk = uart.read(UART_READ_SIZE)
        if chunk:
            print_uart(chunk)
...
def run(payload_path=PAYLOAD, print_tail=True):
    print("WDOG_OLED: release_displays")
    displayio.release_displays()

    print("WDOG_OLED: uploading %s" % BITSTREAM)
