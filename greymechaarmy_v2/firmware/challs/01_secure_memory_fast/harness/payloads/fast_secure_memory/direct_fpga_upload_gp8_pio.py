...
print("DIRECT_FPGA_UPLOAD_GP8_PIO_START")
...
try:
    ...
    # Match default_overlay.set_mode((0, 1, 0)) without constructing Overlay.
    # Overlay indexes reversed: GP15 <- mode[0], GP14 <- mode[1], GP13 <- mode[2].
...
"""
    ...
    for bit in [0, 1, 0, 1, 0]:
        ...
        time.sleep(0.35)
        value = read_pmod()
        print("pio gp8", bit, "addr", 30 | bit, "value", value, fmt(value))
finally:
    if sm:
        sm.deinit()
    if gp8_dio:
...
print("DIRECT_FPGA_UPLOAD_GP8_PIO_DONE")
