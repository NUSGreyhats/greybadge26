...
Options:
  ...
  --expect-return-ready       Require returning-payload DONE and next READY output
...
bash scripts/compile_payload.sh "$payload_arg" "build/payloads/$payload_name" >/dev/null
...
if [[ "$expect_return_ready" -eq 1 ]]; then
  vvp_args+=("+EXPECT_RETURN_READY")
fi
