...
if ($BuildBitstream) {
    wsl bash -lc "bash hardware/scripts/build_greymecha_smoke.sh"
}
...
$buildOutput = wsl bash -lc "bash tools/payload/build_payload.sh '$Payload'"
...
if (-not (Test-Path $bitstreamPath)) {
    throw "Bitstream not found: $bitstreamPath. Run with -BuildBitstream or build hardware/scripts/build_greymecha_smoke.sh first."
}
